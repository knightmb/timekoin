...
set_time_limit(30);
session_name("tkbillfoldservertask");
session_start();
...
function transaction_queue()
{
	...
	if($transaction_queue_hash_different > 0)
	{
		...
		for ($i = 1; $i < $transaction_queue_hash_different + 1; $i++)
		{
			...
			$poll_peer = filter_sql(poll_peer($ip_address, $domain, $subfolder, $port_number, 15000000, "queueclerk.php?action=queue&client=api"));
			...
			while(empty($current_hash) == FALSE)
			{
				...
				if(empty($hash_match) == TRUE)
				{
					...
					$poll_hash = poll_peer($ip_address, $domain, $subfolder, $port_number, 15000, "queueclerk.php?action=transaction&number=$current_hash&client=api");
...
function peer_list()
{
	...
	ini_set('default_socket_timeout', 2); // Timeout for request in seconds
	$db_connect = mysqli_connect(MYSQL_IP,MYSQL_USERNAME,MYSQL_PASSWORD,MYSQL_DATABASE);
	$max_active_peers = mysql_result(mysqli_query($db_connect, "SELECT field_data FROM `options` WHERE `field_name` = 'max_active_peers' LIMIT 1"));
	$max_new_peers = mysql_result(mysqli_query($db_connect, "SELECT field_data FROM `options` WHERE `field_name` = 'max_new_peers' LIMIT 1"));
	...
	if(rand(1,3) == 3)// Randomize to avoid spamming DB
	{
...
//***********************************************************************************
//***********************************************************************************
function clean_inactive_accounts()
{
	if(rand(1,500) == 100)// Randomize to avoid DB Spamming
	{
		$db_connect = mysqli_connect(MYSQL_IP,MYSQL_USERNAME,MYSQL_PASSWORD,MYSQL_DATABASE);
		...
		{
			// Delete all inactive accounts that were never confirmed within 24 hours
			$time = time() - 86400; // Last 24 Hours
			$sql = "SELECT username FROM `users` WHERE `status` > 999 AND `timestamp` < $time";
			$sql_result = mysqli_query($db_connect, $sql);
			$sql_num_results = mysqli_num_rows($sql_result);
			...
			{
				$sql_row = mysqli_fetch_array($sql_result);
				$delete_username = $sql_row["username"];
				
				// Remove cache data templates
				$sql = "DELETE FROM `data_cache` WHERE `data_cache`.`username` = '$delete_username'";

				// Remove user account
				$sql2 = "DELETE FROM `users` WHERE `users`.`username` = '$delete_username'";

				if(mysqli_query($db_connect, $sql) == TRUE && mysqli_query($db_connect, $sql2) == TRUE)
				{
					write_log("Unconfirmed New User Account<br>[$delete_username]<br>Was Deleted From the Database","S");
				}
				else
				{
					write_log("ERROR Removing Unconfirmed New User Account<br>[$delete_username]<br>From the Database!","S");
				}
			}
		}
	}
...
{
	$db_connect = mysqli_connect(MYSQL_IP,MYSQL_USERNAME,MYSQL_PASSWORD,MYSQL_DATABASE);
	$task_status = mysql_result(mysqli_query($db_connect, "SELECT field_data FROM `main_loop_status` WHERE `field_name` = 'task_status' LIMIT 1"));
	$task_active_time = mysql_result(mysqli_query($db_connect, "SELECT field_data FROM `main_loop_status` WHERE `field_name` = 'task_active_time' LIMIT 1"));

	if($task_status == "")
	{
		// Task data blank or missing, create a new one
		$sql = "DELETE FROM `main_loop_status` WHERE `main_loop_status`.`field_name` = 'task_status'";
		mysqli_query($db_connect, $sql);

		$sql = "DELETE FROM `main_loop_status` WHERE `main_loop_status`.`field_name` = 'task_active_time'";
		mysqli_query($db_connect, $sql);

		$time = time();
		$sql = "INSERT INTO `main_loop_status` (`field_name`, `field_data`) VALUES ('task_status', '0')";
		mysqli_query($db_connect, $sql);

		$sql = "INSERT INTO `main_loop_status` (`field_name`, `field_data`) VALUES ('task_active_time', '$time')";
		mysqli_query($db_connect, $sql);
		return;
	}
	...
	if($task_status == 1)
	{
		// Task is already running, how long has it been since it was started?
		$time_active = time() - $task_active_time;

		if($time_active > 30)
		{
			// More than 30 seconds usually means something happened or it timed out after 30 seconds
			$sql = "DELETE FROM `main_loop_status` WHERE `main_loop_status`.`field_name` = 'task_status'";
			mysqli_query($db_connect, $sql);
		...
		{
			// Busy working in another thread
			return;
		}
	}
	else
	{
		// Flag database for working now
		$sql = "UPDATE `main_loop_status` SET `field_data` = '1' WHERE `main_loop_status`.`field_name` = 'task_status' LIMIT 1";
		mysqli_query($db_connect, $sql);

		$time = time();
		$sql = "UPDATE `main_loop_status` SET `field_data` = '$time' WHERE `main_loop_status`.`field_name` = 'task_active_time' LIMIT 1";
		mysqli_query($db_connect, $sql);
	}
	...
	if(rand(1,250) == 100) // Check for updates
	{
	...
	if(mt_rand(1,500) == 100) // Randomize a little to save DB usage
	{
		...
		if($activity_log_count > $activity_log_max)
		{
			...
			write_log("Activity Log Purged of the Last [" . ($activity_log_count - $activity_log_max) . "] Oldest Records", "S");
		}
	}

	// Flag database for working complete
	$sql = "UPDATE `main_loop_status` SET `field_data` = '0' WHERE `main_loop_status`.`field_name` = 'task_status' LIMIT 1";
	mysqli_query($db_connect, $sql);
	return;
}
//***********************************************************************************
