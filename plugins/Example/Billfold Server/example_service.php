...
$already_active = mysql_result(mysqli_query($db_connect, "SELECT field_data FROM `main_loop_status` WHERE `field_name` = 'TKBS_example_service.php' LIMIT 1"));

if($already_active == "")
{
	// Creating Status State - Timekoin Looks for the filename
	mysqli_query($db_connect, "INSERT INTO `main_loop_status` (`field_name` ,`field_data`) VALUES ('TKBS_example_service.php', '1')"); // Active
}
else
{
...
while(1) // Begin Infinite Loop :)
{
	...
	$tkclient_active = mysql_result(mysqli_query($db_connect, "SELECT field_data FROM `main_loop_status` WHERE `field_name` = 'TKBS_example_service.php' LIMIT 1"));
