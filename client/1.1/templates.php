...
<div id="header">
<ul id="top-navigation">
<li><a href="index.php?menu=home" onclick="showWait()" <?PHP echo $home; ?>>Home</a></li>
<li><a href="index.php?menu=address" onclick="showWait()" <?PHP echo $address; ?>>Address Book</a></li>
<li><a href="index.php?menu=peerlist" onclick="showWait()" <?PHP echo $peerlist; ?>>Peerlist</a></li>
<li><a href="index.php?menu=queue" onclick="showWait()" <?PHP echo $queue; ?>>Queue</a></li>
<li><a href="index.php?menu=send" onclick="showWait()" <?PHP echo $send; ?>>Send</a></li>
<li><a href="index.php?menu=history" onclick="showWait()" <?PHP echo $history; ?>>History</a></li>
<li><a href="index.php?menu=options" onclick="showWait()" <?PHP echo $options; ?>>Options</a></li>
<li><a href="index.php?menu=backup" onclick="showWait()" <?PHP echo $backup; ?>>Backup</a></li>
<li><a href="index.php?menu=tools" onclick="showWait()" <?PHP echo $tools; ?>>Tools</a></li>
...
<tr><td align="right"><input type="submit" onclick="showWait()" name="Submit2" value="Save Options" /></FORM>
</td><td style="width:215px" valign="bottom" align="right"><FORM ACTION="index.php?menu=options&upgrade=check" METHOD="post"><input type="submit" onclick="showWait()" name="Submit3" value="Check for Updates" /></FORM></td></tr>
</table>
';
...
function send_receive_body($fill_in_key, $amount, $cancel = FALSE, $easy_key, $message, $name)
{
	if($cancel == TRUE)
	{
		...
		$cancel_button = '<FORM ACTION="index.php?menu=send" METHOD="post"><input type="submit" onclick="showWait()" name="Submit2" value="Cancel" /></FORM>';
	...
	else
	{
		$cancel_button = '<FORM ACTION="index.php?menu=send&easykey=grab" METHOD="post"><input type="text" size="24" name="easy_key" value="' . $easy_key . '" /></br>
			<input type="submit" onclick="showWait()" value="Find Easy Key" /></FORM>';
...
<input type="submit" onclick="showWait()" name="Submit1" value="Send Timekoins" /></FORM></td>
...
function backup_body($private_key, $public_key, $cancel_private = FALSE, $cancel_public = FALSE)
{
	if($cancel_private == TRUE)
	{
		...
		$private_cancel_button = '<FORM ACTION="index.php?menu=backup" METHOD="post"><input type="submit" onclick="showWait()" value="Cancel" /></FORM>';
	...
	{
		// Redo menu to show cancel or complete buttons
		$public_cancel_button = '<FORM ACTION="index.php?menu=backup" METHOD="post"><input type="submit" onclick="showWait()" value="Cancel" /></FORM>';
...
return '<table border="0" cellpadding="6"><tr><td colspan="2"><strong><font color="blue">Restore Private Key</font></strong></td></tr>
			...
			<tr><td><input type="submit" onclick="showWait()" value="Restore Private Key"/></FORM>' . $are_you_sure . '</td><td align="left" valign="top">' . $private_cancel_button . '</td></tr>
			...
			<tr><td><input type="submit" onclick="showWait()" value="Restore Public Key"/></FORM>' . $are_you_sure2 . '<td align="left" valign="top">' . $public_cancel_button . '</td></tr></table>';
}
//***********************************************************
//***********************************************************
