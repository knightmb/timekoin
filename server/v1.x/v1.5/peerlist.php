...
if($active_peers < $max_active_peers)
{
	...
	for ($i = 0; $i < $sql_num_results; $i++)
	{
		...
		if($duplicate_peer == FALSE)
		{
			...
			$poll_peer = filter_sql(file_get_contents("http://$site_address:$port_number/$subfolder/peerlist.php?action=poll&challenge=$poll_challenge", FALSE, $context, NULL, 10));
			...
			if($poll_peer == $hash_solution)
			{
				...
				$poll_peer = filter_sql(file_get_contents("http://$site_address:$port_number/$subfolder/peerlist.php?action=join", FALSE, $context, NULL, 10));

				if($poll_peer == "OK")
				{
					// Add this peer to the active list
					$poll_peer = filter_sql(file_get_contents("http://$site_address:$port_number/$subfolder/peerlist.php?action=exchange&domain=$my_server_domain&subfolder=$my_server_subfolder&port_number=$my_server_port_number", FALSE, $context, NULL, 512));
...
if($new_peers_numbers < $max_new_peers && rand(1,3) == 2)//Randomize a little to avoid spamming for new peers
{
	...
	for ($i = 0; $i < $sql_num_results; $i++)
	{
		...
		$poll_peer = filter_sql(file_get_contents("http://$site_address:$port_number/$subfolder/peerlist.php?action=new_peers", FALSE, $context, NULL, 10000));
