...
//****************************************************************************	
	if($_GET["menu"] == "peerlist")
	{
		...
		else
		{
			...
			$quick_info = 'Shows all Active Peers.</br></br>You can manually delete or edit peers in this section.</br></br>Peers in <font color="blue">Blue</font> will not expire after 5 minutes of inactivity.';
	...
	if($_GET["menu"] == "system")
	{
		...
		if($_GET["time"] == "poll")
		{
			...
			for ($i = 0; $i < $sql_num_results; $i++)
			{
				...
				$poll_peer = filter_sql(file_get_contents("http://$site_address:$port_number/$subfolder/peerlist.php?action=polltime", FALSE, $context, NULL, 12));
				...
				$body_string = $body_string . '<tr><td class="style2"><p style="word-wrap:break-word; font-size:12px;">' . $site_address . ':' . $port_number . '/' . $subfolder . '</p></td>';
				$body_string = $body_string . '<td class="style2"><p style="font-size:12px;">' . $poll_peer . '</p></td>';
				$body_string = $body_string . '<td class="style2"><p style="font-size:12px;">' . $variance . '</p></td></tr>';
			...
			$body_string = $body_string . '<strong>Variance Average: ' . $variance_average . '</strong></br></br>';
		}
		else
		{
...
//****************************************************************************
	if($_GET["menu"] == "options")
	{
		...
		if($_GET["menu"] == "options" && $_GET["refresh"] == "change")
		{
			...
			{
				$body_text = $body_text . '<font color="blue"><strong>Refresh Settings & Hash Code Update Saved!</strong></font></br>';
			}
			else
			{
				$body_text = $body_text . '<strong>Refresh / Hash Code Update ERROR...</strong></br>';
			}
...
//****************************************************************************	
	if($_GET["menu"] == "generation")
	{
		...
		if($_GET["generate"] == "showqueue")
		{
			...
			$body_string = $body_string . '<hr></hr><strong>Election Queue List</strong>
		...
		$quick_info = 'You must remain online to generate currency.</br></br>The longer your server participates, the more it will be allowed to generate as time progresses.</br></br>
			...
			Next Election in ' . $time_election . '</strong></font>';
	...
	if($_GET["menu"] == "send")
	{
		...
		else
		{
			...
			else
			{
				...
				$display_balance = db_cache_balance($my_public_key);
				$body_string = send_receive_body($easy_key, NULL, NULL, $last_easy_key);
				$body_string = $body_string . $server_message;
			}
		}
	...
	if($_GET["menu"] == "tools")
	{
		...
		if(empty($_GET["action"]) == TRUE)
		{
			...
			$body_string = '<strong>Showing Last <font color="blue">' . $show_last . '</font> Log Events</strong>' . $filter_by . '<table border="0" cellspacing="5"><tr><td>
				...
				<option value="R">Generation Request</option><option value="MA">Main Program</option><option value="QC">Transaction Queue Clerk</option>
				<option value="TC">Transaction Clerk</option><option value="T">Transactions</option><option value="TR">Treasurer Processor</option></select></td></tr></table>
			...
			$body_string = $body_string . '<tr><td><input type="text" size="5" name="show_more_logs" value="' . $show_last .'" /><input type="submit" name="show_last" value="Show Last" /></FORM></td>
