...
//***********************************************************************************
	...
	if($sql_num_results > 0)
	{
		...
		for ($i = 0; $i < $sql_num_results; $i++)
		{
			...
			$poll_peer = file_get_contents("http://$site_address:$port_number/$subfolder/genpeer.php?action=gen_hash", NULL, NULL, NULL, 65);
	...
	if($gen_list_hash_different > $gen_list_hash_match)
	{
		...
		if($generation_peer_list_no_sync > 15)
		{
			// Our generation peer list is how of sync for a long time, clear to list to start over
		...
		for ($i = 1; $i < $gen_list_hash_different + 1; $i++)
		{
			...
			$poll_peer = file_get_contents("http://$site_address:$port_number/$subfolder/genpeer.php?action=gen_peer_list", NULL, NULL, NULL, 100000);

			// Bring up first match (if any) to compare agaist our database
			preg_match_all('|-----public_key1=(.*?)-----join1=|', $poll_peer, $matches);
			foreach($matches[1] as $current_public_key)
			{
			}

			//Convert from base64
			$current_public_key = base64_decode($current_public_key);

			$match_number = 1;
			$last_public_key = 0;

			while($current_public_key !== $last_public_key)
			{
				$last_public_key = $current_public_key; // Duplicate Check

				//Check if this public key is already in our peer list
				$public_key_match = mysql_result(mysql_query("SELECT * FROM `generating_peer_list` WHERE `public_key` = '$current_public_key' LIMIT 1"),0,"join_peer_list");
				
				if(empty($public_key_match) == FALSE)
				{
					// A match means both I and the peer have the same public key in the peer list
				}
				else
				{
					// This peer has an unknown public key, store in our database
					preg_match_all("|-----public_key$match_number=(.*)-----join$match_number=|", $poll_peer, $public_key_matches);
					foreach($public_key_matches[1] as $gen_peer_public_key)
					{
					}
					preg_match_all("|-----join$match_number=(.*)-----last$match_number=|", $poll_peer, $public_key_matches);
					foreach($public_key_matches[1] as $gen_peer_join_peer_list)
					{
					}
					preg_match_all("|-----last$match_number=(.*)-----END$match_number|", $poll_peer, $public_key_matches);
					foreach($public_key_matches[1] as $gen_peer_last_generation)
					{
					}
				...
				preg_match_all("|-----public_key$match_number=(.*?)-----join$match_number=|", $poll_peer, $matches);
				foreach($matches[1] as $current_public_key)
				{
				}
