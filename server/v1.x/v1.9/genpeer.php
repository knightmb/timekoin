...
//***********************************************************************************
	...
	if($sql_num_results > 0)
	{
		...
		for ($i = 0; $i < $sql_num_results; $i++)
		{
			...
			$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/genpeer.php?action=gen_hash", FALSE, $context, NULL, 100));
	...
	if($gen_list_hash_different > $gen_list_hash_match)
	{
		...
		$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/genpeer.php?action=gen_peer_list", FALSE, $context, NULL, 100000));
