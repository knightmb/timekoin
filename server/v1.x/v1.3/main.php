...
if($_GET["action"]=="begin_main")
{
	...
	if($main_heartbeat_active == FALSE && $datbase_error == FALSE)
	{
		...
		// Record when started
		$start_time = mysql_result(mysql_query("SELECT * FROM `options` WHERE `field_name` = 'timekoin_start_time' LIMIT 1"),0,"field_data");
...
function loop()
{
	...
	unset($sql_result);
	unset($sql_num_results);
	unset($request_max);
	unset($sql_num_results2);
	unset($select_IP);
	unset($sql_row);
	unset($no_peer_activity);
	unset($transclerk_loop);
	unset($generation_loop);
	unset($treasurer_loop);
	unset($peerlist_loop);
	unset($queueclerk_loop);
	unset($genpeer_loop);
	unset($transclerk_loop);
	unset($watchdog_loop_active);
	unset($watchdog_last_heartbeat);
