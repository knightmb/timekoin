...
if($loop_active == 0)
{
	...
	$sql = "UPDATE `main_loop_status` SET `field_data` = '1' WHERE `main_loop_status`.`field_name` = 'treasurer_heartbeat_active' LIMIT 1";
	mysql_query($sql);
}
else
{
...
if($sql_num_results > 0)
{
	...
	// Not allowed 80 seconds before and 30 seconds after generation cycle.
	if(($next_generation_cycle - time()) > 80 && (time() - $current_generation_cycle) > 30)
	{
			...
			else
			{
				...
				if(empty($found_public_key_queue) == TRUE)
				{
					if($firewall_blocked == "1")
					{
						...
						$poll_peer =  file_get_contents("http://$site_address:$port_number/$subfolder/queueclerk.php?action=input_transaction", false, $context);
					...
					else
					{
						...
						VALUES ('" . time() . "', '$public_key', '$crypt1', '$crypt2' , '$crypt3', '$hash_check' , '$attribute');";			
		...
		if($sql_row["attribute"] == "G") // Currency Generation Transaction
		{
			...
			if($last3_gen + $tk_random_number < 10)
			{
				...
				if(empty($generation_public_key) == TRUE)
				{
					...
					write_log("Key Not in Generation Peer List:" . substr(base64_encode($public_key), 0, 200), "G");
				}
				else
				{
					...
					if(empty($found_public_key_queue) == TRUE)
					{
						...
						if((time() - $join_peer_list) >= 3600) // It's been more than 3600 seconds since this public key joined the generating peer list
						{
							...
							foreach($matches[1] as $transaction_amount_sent)
							{
							}
							...
							if(peer_gen_amount($public_key) == $transaction_amount_sent && $amount_valid == TRUE)
							{
							...
							if(hash('sha256', $crypt1 . $crypt2 . $crypt3) == $hash_check && $amount_valid == TRUE) // Hash check for tampering
							{
								...
								{
									//Something didn't work
									write_log("Generation Insert Failed for this Key:" . substr(base64_encode($public_key), 0, 200), "G");
								}
								...
								{
									//Something didn't work
									write_log("Generation Timestamp Update Failed for this Key:" . substr(base64_encode($public_key), 0, 200), "G");
								}
							}
							else
							{
								// Failed Hash check or Valid Amount check
								write_log("Generation Hash/Amount Check Failed for this Key:" . substr(base64_encode($public_key), 0, 200), "G");
							}
						}
						else
						{
							// Not enough time has passed
							write_log("Generation Too Early for this Key:" . substr(base64_encode($public_key), 0, 200), "G");
						}
					}
					else
					{
						// Duplicate generation transaction already exist
						write_log("Generation Duplicate Discarded for this Key:" . substr(base64_encode($public_key), 0, 200), "G");						
					}
		...
		if($sql_row["attribute"] == "T") // Regular Transaction
		{
			...
			if(empty($found_public_key_queue) == TRUE)
			{
				...
				if(check_crypt_balance($public_key) >= $transaction_amount_sent && $transaction_amount_sent > 0 && $amount_valid == TRUE)
				{
					...
					if(hash('sha256', $crypt1 . $crypt2 . $crypt3) == $hash_check)
					{
						...
						if(strlen($public_key) > 256 && strlen($public_key_to) > 256 && $public_key !== $public_key_to) // Filter to/from self public keys
						{
							...
							{
								//Something didn't work
								write_log("Transaction Insert Failed for this Key:" . substr(base64_encode($public_key), 0, 200), "T");
							}
						}
						else
						{
							// Invalid or blank Public Key(s)
							write_log("Transaction Public Key Error for this Key:" . substr(base64_encode($public_key), 0, 200), "T");
					...
					{
						// Hash check failed
						write_log("Transaction Hash Check Failed for this Key:" . substr(base64_encode($public_key), 0, 200), "T");
						$safe_delete_transaction = TRUE;						
					}				
				}
				else
				{
					...
					write_log("Transaction Balance Check Failed for this Key:" . substr(base64_encode($public_key), 0, 200), "T");
					$safe_delete_transaction = TRUE;
				}
			}
			else
			{
				...
				write_log("Duplicate Transaction Failed for this Key:" . substr(base64_encode($public_key), 0, 200), "T");
				$safe_delete_transaction = TRUE;
			}
...
$sql = "UPDATE `main_loop_status` SET `field_data` = '" . time() . "' WHERE `main_loop_status`.`field_name` = 'treasurer_last_heartbeat' LIMIT 1";
mysql_query($sql);
