...
define("TIMEKOIN_VERSION","1.0"); // Software Version
...
{
	$sql_log = "INSERT INTO `timekoin`.`activity_logs` (`timestamp` ,`log` ,`attribute`)
...
function check_crypt_balance_range($public_key, $block_start = 0, $block_end = 0)
{
	...
	for ($i = 0; $i < $sql_num_results; $i++)
	{
		...
		if($attribute == "G" && $public_key_from == $public_key_to && $hash == hash('sha256', $crypt1 . $crypt2 . $crypt3))
		{
			...
			foreach($matches[1] as $transaction_hash)
			{
			}				
			...
			if($transaction_hash == hash('sha256', $crypt1 . $crypt2))
			{
				...
				$crypto_balance = $crypto_balance + $transaction_amount_sent;
			}
		}
...
function check_crypt_balance($public_key)
{
	...
	else
	{
		...
		$previous_foundation_block = $sql_row["block"];
...
function peer_gen_amount($public_key)
{
	...
	if($join_peer_list == "" || $join_peer_list < TRANSACTION_EPOCH)
	{
	...
	else
	{
		...
		//return "Join:$join_peer_list Peer Age Weeks: " . $peer_age;
		...
		if($peer_age > 1)
		{
			// Use calculation based on the double of weeks per +1 generation increase per double
			$koin_counter = -1;
...
function scorePublicKey($public_key)
{
	...
	for ($i = 0; $i < 18; $i++)
	{
		...
		$public_key_score = $public_key_score + getCharFreq($public_key, $character);
	}
...
function send_timekoins($my_private_key, $my_public_key, $send_to_public_key, $amount, $message)
{
	...
	else
	{
		...
		// Trim any message to 64 characters max
...
function visual_walkhistory($block_start = 0, $block_end = 0)
{
	...
	for ($i = $block_start; $i < $block_end; $i++)
	{
		...
		if($next_timestamp != $timestamp)
		{
			...
			$wrong_timestamp++;
			$wrong_block_numbers = $wrong_block_numbers . " " . $i;
		}
		...
		$output = $output . '</br>Timestamp in Database: ' . $timestamp;
		$output = $output . '</br>Calculated Hash: ' . $my_hash;
		$output = $output . '</br>&nbsp;Database Hash : ' . $next_hash;

		if($my_hash == $next_hash)
		{
			$output = $output . '</br><font color=green>Hash Match...</font>';
		}
		else
		{
			$output = $output . '</br><strong><font color=red>Hash MISMATCH</font></strong></td></tr>';
			$wrong_hash++;
	...
	$output = $output . '<tr><td class="style2"><strong><font color="blue">Total Wrong Sequence: ' . $wrong_timestamp . '</strong></font>';
	$output = $output . '</br><strong><font color="red">Blocks Wrong:</font> ' . $wrong_block_numbers . '</strong></td></tr>';
	$output = $output . '<tr><td class="style2"><strong><font color="blue">Total Wrong Hash: ' . $wrong_hash . '</strong></font>';
	$output = $output . '</br><strong><font color="red">Blocks Wrong:</font> ' . $wrong_hash_numbers . '</strong></td></tr>';	
...
function visual_repair($block_start = 0)
{
	...
	if(mysql_query($sql) == TRUE)
	{
		$output = $output . '<tr><td class="style2">Clearing Hash Timestamps Ahead of Block #' . $block_start . '</td></tr>';
	}
	else
	{
	...
	{
		$output = $output . "<tr><td><strong>Repairing Block# $t</strong>";
		...
		{
			// Something failed
			$output = $output . '</br><strong><font color="red">Repair ERROR in Database</font></strong></td></tr>';
		}
		else
		{
			$output = $output . '</br><strong><font color="blue">Repair Complete...</font></strong></td></tr>';
		}
	} // End for loop
