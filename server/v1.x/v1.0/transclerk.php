...
$foundation_active = mysql_result(mysql_query("SELECT * FROM `main_loop_status` WHERE `field_name` = 'foundation_heartbeat_active' LIMIT 1"),0,"field_data");
...
// Update transaction history hash
		$current_history_hash = mysql_result(mysql_query("SELECT * FROM `options` WHERE `field_name` = 'transaction_history_hash' LIMIT 1"),0,"field_data");
		$transaction_history_block_check = mysql_result(mysql_query("SELECT * FROM `options` WHERE `field_name` = 'transaction_history_block_check' LIMIT 1"),0,"field_data");
		$foundation_block_check = mysql_result(mysql_query("SELECT * FROM `options` WHERE `field_name` = 'foundation_block_check' LIMIT 1"),0,"field_data");
		...
		if($transaction_history_block_check != "0" || $foundation_block_check == "1")
		{
			...
			if($transaction_history_block_check > 0)
			{
...
//***********************************************************************************
	...
	if($sql_num_results > 0)
	{
		...
		for ($i = 0; $i < $sql_num_results; $i++)
		{
			...
			$poll_peer = file_get_contents("http://$site_address:$port_number/$subfolder/transclerk.php?action=history_hash", NULL, NULL, NULL, 65);
...
//***********************************************************************************
	...
	if($trans_list_hash_different > $trans_list_hash_match)
	{
		...
		{
			$hash_number = mysql_result(mysql_query("SELECT * FROM `options` WHERE `field_name` = 'block_check_start' LIMIT 1"),0,"field_data");
		...
		if(($hash_number + $hash_check_counter) >= $current_generation_block)
		{
			...
			if($hash_check_counter == 0)
			{
				...
				$sql = "UPDATE `options` SET `field_data` = '$hash_number_back_database' WHERE `options`.`field_name` = 'block_check_back' LIMIT 1";				
				mysql_query($sql);
			}
		}
		...
		for ($h = 0; $h < $hash_check_counter; $h++)
		{
			...
			for ($i = 1; $i < $trans_list_hash_different + 1; $i++)
			{
				...
				$poll_peer = file_get_contents("http://$site_address:$port_number/$subfolder/transclerk.php?action=block_hash&block_number=$hash_number", NULL, NULL, NULL, 65);
...
//***********************************************************************************
			...
			if($hash_disagree > $hash_agree)
			{
				...
				else
				{
					...
					$poll_peer = file_get_contents("http://$site_address:$port_number/$subfolder/transclerk.php?action=transaction_data&block_number=$block_number", NULL, NULL, NULL, 200000);

					$transaction_hash = 1;
					$last_hash = 0;
					$tc = 1;

					while($last_hash !== $transaction_hash && empty($poll_peer) == FALSE)
					{
						$last_hash = $transaction_hash;
						
						preg_match_all("|-----timestamp$tc=(.*)-----public_key_from$tc=|", $poll_peer, $hash_matches);
						foreach($hash_matches[1] as $transaction_timestamp)
						{
						}
						preg_match_all("|-----public_key_from$tc=(.*)-----public_key_to$tc=|", $poll_peer, $hash_matches);
						foreach($hash_matches[1] as $transaction_public_key_from)
						{
						}
						preg_match_all("|-----public_key_to$tc=(.*)-----crypt1data$tc=|", $poll_peer, $hash_matches);
						foreach($hash_matches[1] as $transaction_public_key_to)
						{
						}
						preg_match_all("|-----crypt1data$tc=(.*)-----crypt2data$tc=|", $poll_peer, $hash_matches);
						foreach($hash_matches[1] as $transaction_crypt1)
						{
						}
						preg_match_all("|-----crypt2data$tc=(.*)-----crypt3data$tc=|", $poll_peer, $hash_matches);
						foreach($hash_matches[1] as $transaction_crypt2)
						{
						}
						preg_match_all("|-----crypt3data$tc=(.*)-----hash$tc=|", $poll_peer, $hash_matches);
						foreach($hash_matches[1] as $transaction_crypt3)
						{
						}
						preg_match_all("|-----hash$tc=(.*)-----attribute$tc=|", $poll_peer, $hash_matches);
						foreach($hash_matches[1] as $transaction_hash)
						{
						}
						preg_match_all("|-----attribute$tc=(.*)-----end$tc|", $poll_peer, $hash_matches);
						...
						$found_duplicate = mysql_result(mysql_query("SELECT * FROM `transaction_history` WHERE `timestamp` = '$transaction_timestamp' AND `public_key_from` = '$transaction_public_key_from' AND `hash` = '$transaction_hash' LIMIT 1"),0,"timestamp");

						if(empty($found_duplicate) == TRUE && $last_hash !== $transaction_hash)
						{
							$sql = "INSERT INTO `transaction_history` (`timestamp`,`public_key_from`,`public_key_to`,`crypt_data1`,`crypt_data2`,`crypt_data3`, `hash`, `attribute`)
			...
			else if($double_check_block == TRUE && $double_check_counter >= 2)
			{
				...
				write_log("Too Much Peer Conflict for Block#$block_number. This block will be remain empty until repaired.", "TC");				
			}
			else
			{
...
//***********************************************************************************
		...
		if($error_check_active == TRUE && $foundation_block_check != "1") // Reset transaction history hash after error check completes
		{
			$sql = "SELECT * FROM `transaction_history` WHERE `attribute` = 'H' ORDER BY `timestamp`";
			$sql_result = mysql_query($sql);
			$sql_num_results = mysql_num_rows($sql_result);
			...
			write_log("History Check Complete. No Errors Found.", "TC");
		}
	...
	else
	{
		...
		if(rand(1,3) == 2)
		{
			...
			if($sql_num_results > 0)
			{
				...
				$poll_peer = file_get_contents("http://$site_address:$port_number/$subfolder/transclerk.php?action=block_hash&block_number=$random_block", NULL, NULL, NULL, 65);
				...
				if(empty($poll_peer) == FALSE && strlen($poll_peer) > 32)
				{
					...
					else if($poll_peer !== $random_hash_build && empty($poll_peer) == FALSE)
					{
						...
						write_log("One of my Peers Reports that My Block#$random_block is Wrong. Will Double Check with other Peers.", "TC");
					}
