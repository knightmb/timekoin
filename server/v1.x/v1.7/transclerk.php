...
//***********************************************************************************
	...
	if($sql_num_results > 0)
	{
		...
		for ($i = 0; $i < $sql_num_results; $i++)
		{
			...
			$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/transclerk.php?action=history_hash", FALSE, $context, NULL, 65));
	...
	if($trans_list_hash_different > $trans_list_hash_match)
	{
		...
		for ($h = 0; $h < $hash_check_counter; $h++)
		{
			...
			for ($i = 1; $i < $trans_list_hash_different + 1; $i++)
			{
				...
				$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/transclerk.php?action=block_hash&block_number=$hash_number", FALSE, $context, NULL, 65));
...
//***********************************************************************************
			...
			if($hash_disagree > $hash_agree)
			{
				...
				else
				{
					...
					$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/transclerk.php?action=transaction_data&block_number=$block_number", FALSE, $context, NULL, 200000));
	...
	else
	{
		...
		if(rand(1,4) == 2)
		{
			...
			if($sql_num_results > 0)
			{
				...
				$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/transclerk.php?action=block_hash&block_number=$random_block", FALSE, $context, NULL, 65));
