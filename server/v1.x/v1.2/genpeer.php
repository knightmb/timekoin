	...
	if($last3_gen + $tk_random_number > 14)
	{
		...
		if($sql_num_results > 0)
		{
			if($sql_num_results == 1)
			{
				...
				write_log("Generation Peer Elected for Public Key: " . substr(base64_encode($public_key), 0, 200), "GP");				
			}
			else
			{
				...
				write_log("Generation Peer Elected for Public Key: " . substr(base64_encode($public_key_winner), 0, 200), "GP");
...
//***********************************************************************************
	...
	if($sql_num_results > 0)
	{
		...
		for ($i = 0; $i < $sql_num_results; $i++)
		{
			...
			$poll_peer = file_get_contents("http://$site_address:$port_number/$subfolder/genpeer.php?action=gen_hash", NULL, NULL, NULL, 100);
	...
	if($gen_list_hash_different > $gen_list_hash_match)
	{
		...
		for ($i = 1; $i < $gen_list_hash_different + 1; $i++)
		{
			...
			while($current_public_key !== $last_public_key)
			{
				...
				$public_key_match = mysql_result(mysql_query("SELECT * FROM `generating_peer_list` WHERE `public_key` = '$gen_peer_public_key' LIMIT 1"),0,"join_peer_list");
				...
				$current_public_key = $gen_peer_public_key;
				$match_number++;
		...
		for ($i = 0; $i < $sql_num_results; $i++)
		{
			...
			if(empty($public_key_found_timestamp) == TRUE && empty($public_key_found_peer) == TRUE && $timestamp >= $current_generation_cycle)
			{
				...
				if($transaction_info == $crypt2)
				{
					...
					write_log("Generation Peer Queue List was updated with Public Key: " . substr(base64_encode($public_key), 0, 200), "GP");
				}
			}
