...
if($loop_active == "0")
{
	...
	$sql = "UPDATE `main_loop_status` SET `field_data` = '1' WHERE `main_loop_status`.`field_name` = 'foundation_heartbeat_active' LIMIT 1";
	...
	if($sql_num_results > 0)
	{
		...
		if(empty($current_foundation_hash) == FALSE)
		{
			for ($i = 0; $i < $sql_num_results; $i++)
			{
				...
				$poll_peer = file_get_contents("http://$site_address:$port_number/$subfolder/foundation.php?action=block_hash&block_number=$rand_block", NULL, NULL, NULL, 65);
			...
			else
			{
				...
				if($foundation_hash_different == $sql_num_results)
				{
					$repair_block = TRUE;
				}
			}
			...
			if($repair_block == TRUE)
			{
				...
				if(mysql_query($sql) == TRUE)
				{
					...
					if(mysql_query($sql) == TRUE)
					{
						...
						if(mysql_query($sql) == TRUE)
						{
							$sql = "UPDATE `options` SET `field_data` = '$foundation_time_start' WHERE `options`.`field_name` = 'foundation_block_check_start' LIMIT 1";
							$sql2 = "UPDATE `options` SET `field_data` = '$foundation_time_end' WHERE `options`.`field_name` = 'foundation_block_check_end' LIMIT 1";
				...
				if($rebuild_foundation == TRUE)
				{
					...
					if($do_history_walk == 0)
					{
						...
						for ($f = 0; $f < $sql_num_results2; $f++)
						{
							...
							$hash = $hash . $sql_row2["timestamp"] . $sql_row2["public_key_from"] . $sql_row2["public_key_to"] . $sql_row2["hash"] . $sql_row2["attribute"];
						}	
					...
					{
						write_log("Transaction History Walk FAILED. A Transaction History Check has been scheduled to examine Block #$do_history_walk", "FO");
