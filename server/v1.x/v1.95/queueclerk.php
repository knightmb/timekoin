...
if(($next_generation_cycle - time()) > 30 && (time() - $current_generation_cycle) > 30)
{
	...
	if($sql_num_results > 0)
	{
		...
		for ($i = 0; $i < $sql_num_results; $i++)
		{
			...
			$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/queueclerk.php?action=trans_hash", FALSE, $context, NULL, 40));
	...
	if($transaction_queue_hash_different > 0)
	{
		...
		for ($i = 1; $i < $transaction_queue_hash_different + 1; $i++)
		{
			...
			$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/queueclerk.php?action=queue", FALSE, $context, NULL, 8200));
			...
			while(empty($current_hash) == FALSE)
			{
				...
				if(empty($hash_match) == TRUE)
				{
					...
					$poll_hash = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/queueclerk.php?action=transaction&number=$current_hash", FALSE, $context, NULL, 1500));
