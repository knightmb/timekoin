...
if($_GET["action"] == "poll" && empty($_GET["challenge"]) == FALSE)
{
	...
	if(rand(1,30) == 15)
	{
		$allow_ambient_peer_restart = mysql_result(mysql_query("SELECT * FROM `options` WHERE `field_name` = 'allow_ambient_peer_restart' LIMIT 1"),0,"field_data");
			...
			if($script_loop_active > 0)
			{
				...
				if((time() - $script_last_heartbeat) > 30) // Greater than triple the loop time, something is wrong
				{
					...
					else
					{
						...
						exec("php main.php &> /dev/null &"); // This will execute without waiting for it to finish
					}
...
if($active_peers < $max_active_peers)
{
	...
	for ($i = 0; $i < $sql_num_results; $i++)
	{
		...
		if($duplicate_peer == FALSE)
		{
			...
			$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/peerlist.php?action=poll&challenge=$poll_challenge", FALSE, $context, NULL, 10));
			...
			if($poll_peer == $hash_solution)
			{
				...
				$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/peerlist.php?action=join", FALSE, $context, NULL, 10));

				if($poll_peer == "OK")
				{
					// Add this peer to the active list
					$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/peerlist.php?action=exchange&domain=$my_server_domain&subfolder=$my_server_subfolder&port_number=$my_server_port_number", FALSE, $context, NULL, 512));
					...
					if($exchange_status == "OK")
					{
						...
						$peer_domain = find_string("-----domain=", "-----subfolder", $poll_peer);
						$peer_subfolder = find_string("-----subfolder=", "-----port_number", $poll_peer);
						$peer_port_number = find_string("-----port_number=", "-----", $poll_peer);
...
if($new_peers_numbers < $max_new_peers && rand(1,3) == 2)//Randomize a little to avoid spamming for new peers
{
	...
	for ($i = 0; $i < $sql_num_results; $i++)
	{
		...
		$poll_peer = filter_sql(file_get_contents("http$ssl://$site_address:$port_number/$subfolder/peerlist.php?action=new_peers", FALSE, $context, NULL, 10000));
...
// Send a heartbeat to all peers in our list to make sure they are still online
	...
	for ($i = 0; $i < $sql_num_results; $i++)
	{
		...
		if((time() - $last_heartbeat) > 300 && $join_peer_list != 0)
		{
			// Server has been offline too long, remove it from the active peers list
