...
define("TIMEKOIN_VERSION","4.19"); // This Timekoin Software Version
define("NEXT_VERSION","next_version16.txt"); // What file to check for future versions
...
function check_crypt_balance_range($public_key = "", $block_start = 0, $block_end = 0)
{
	...
	if(function_exists('openssl_public_decrypt') == TRUE)
	{
		$try_openssl_public_decrypt = TRUE;
	}
	...
	for ($i = 0; $i < $sql_num_results; $i++)
	{
		...
		if($try_openssl_public_decrypt == TRUE)
		{
			// Use OpenSSL if possible
			openssl_public_decrypt($crypt3, $transaction_info, $public_key_from, OPENSSL_PKCS1_PADDING);
		}
		...
		if(empty($transaction_info) == TRUE)
		{
			// OpenSSL can't decrypt this for some reason
			// Use built in Code instead
			if($last_public_key != $public_key_from)
			{
				// Load the New Public Key
				$rsa->loadKey($public_key_from);
			}

			// Cache key in RAM if used frequently
			$last_public_key = $public_key_from;

			// Decrypt Data
			$transaction_info = $rsa->decrypt($crypt3);
			...
			if(empty($transaction_info) == TRUE)
			{
				...
				// Decrypt Data				
				$transaction_info = $rsa->decrypt($crypt3);
				// Set back to PCKS1 when finished
				$rsa->setEncryptionMode(CRYPT_RSA_ENCRYPTION_PKCS1);
			}			
		}
